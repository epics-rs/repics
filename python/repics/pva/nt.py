...
class _Snap:
    """What an augmented value reads its ``_META`` from: the payload plus
    the metadata an NT structure carried (the pvAccess stand-in for the
    ...
    def __init__(self, value: Any, **meta: Any):
        ...
        self.timestamp = self.raw_stamp[0] + self.raw_stamp[1] * 1e-9
        self.element_count = meta.get("element_count", 1)


def _augmented(cls: type, payload: Any, snap: _Snap) -> Any:
    """An instance of the augmented class ``cls`` over ``payload`` reading
    its metadata from ``snap``; ``augment`` picks the class itself."""
    out = cls(payload)
    out._snap = snap
    return out
...
class NTEnum(NTBase):
    ...
    def unwrap(self, V: Value) -> Any:
        ...
        meta["datatype"] = "enum"
        meta["enums"] = choices
        out = _augmented(ntenum, idx, _Snap(idx, **meta))
