...
| pyepics | epicsrs |
| --- | --- |
| `import epics` | `from epicsrs import pv as epics` (the `PV` class) or `from epicsrs import ca` (the functions) |
| `epics.PV(...)` | `epicsrs.pv.PV(pvname, callback=None, form='time', auto_monitor=None, connection_callback=None, connection_timeout=None, access_callback=None, count=None)` |
| `epics.get_pv(...)` | `epicsrs.pv.get_pv(pvname, form='time', connect=False, timeout=5.0, **kw)`; the cache is keyed by `(pvname, form)` and holds only PVs made through `get_pv` |
| `epics.caget(...)` | `epicsrs.ca.caget(pv, form='time', datatype=None, count=0, timeout=5.0, throw=True)`; returns an augmented value, not a bare Python number |
| `epics.caput(...)` | `epicsrs.ca.caput(pv, value, wait=False, datatype=None, timeout=5.0, repeat_value=False, throw=True)` |
| `epics.camonitor(...)` | `epicsrs.ca.camonitor(pv, callback, ...)` returns a subscription object with `close()`; there is no `camonitor_clear` |
| `epics.cainfo(...)` | `epicsrs.ca.cainfo(pv, wait=True, timeout=5.0, throw=True)` returns a `CAInfo` object instead of printing |
| `epics.ca.*` | `epicsrs.CaContext` and `epicsrs.CaChannel` (see [ca.md](ca.md)); the pyepics `ca` module functions do not exist |
...
```python
from epicsrs import pv as epics      # pyepics: import epics
p = epics.PV("demo:ai")
...
| aioca | epicsrs |
| --- | --- |
| `from aioca import caget, caput, camonitor, cainfo, connect` | `from epicsrs.aio import caget, caput, camonitor, cainfo, connect` |
| `from aioca import CANothing, CAInfo` | `from epicsrs import CaNothing, CAInfo` |
| `FORMAT_RAW`, `FORMAT_TIME`, `FORMAT_CTRL` | the strings `'raw'`, `'time'`, `'ctrl'` in `form=` |
| `DBR_*`, `DBE_*`, `ECA_*` | the same names, exported from `epicsrs` |
...
The error hierarchy is `epicsrs.CaError` with `CaTimeout` and
`CaDisconnected` beneath it; `CaNothing` is falsy and carries `errorcode`.

```python
import asyncio
from epicsrs.aio import caget, caput, camonitor   # aioca: from aioca import ...
async def main():
...
```
thread 'epicsrs-rt' (3606484) panicked at .../pyo3-0.27.2/src/interpreter_lifecycle.rs:117:13:
...
`camonitor` (the `epicsrs.aio` example in [aio.md](aio.md)) did not show
it in 4 runs.
...
| p4p | epicsrs |
| --- | --- |
| `from p4p.client.thread import Context` | `from epicsrs.pva import Context` |
| `from p4p.client.asyncio import Context` | `from epicsrs.pva.asyncio import Context` |
| `from p4p import Value, Type` | `from epicsrs.pva import Value, Type` |
| `from p4p.nt import NTScalar, ...` | `from epicsrs.pva.nt import NTScalar, ...` |
| `from p4p.server import Server, StaticProvider` | `from epicsrs.pva.server import Server, StaticProvider` |
| `from p4p.server.thread import SharedPV` | `from epicsrs.pva.server import SharedPV` |
| `from p4p.server.asyncio import SharedPV` | `from epicsrs.pva.server.asyncio import SharedPV` |
| `from p4p.client.thread import Disconnected, RemoteError, TimeoutError` | `from epicsrs.pva import Disconnected, RemoteError, TimeoutError` (aliases of `PvaDisconnected`, `PvaRemoteError`, `PvaTimeout`) |
...
| Threads | the handler pool is four `WorkQueue` threads created lazily; the monitor dispatcher is one thread named `epicsrs pvmonitor` |
...
```python
from epicsrs.pva import Context          # p4p: from p4p.client.thread import Context
from epicsrs.pva.nt import NTScalar      # p4p: from p4p.nt import NTScalar
from epicsrs.pva.server import Server, StaticProvider, SharedPV   # p4p.server / p4p.server.thread
